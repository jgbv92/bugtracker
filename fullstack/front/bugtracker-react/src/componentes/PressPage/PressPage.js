...
class PressPage extends Component {
    state = { 
     ...
     //"informador" es un valor fijo que se carga a través de una API y se setea como readOnly, 
     ...
     peticionGet = (pageNumber = 1) => {
      ...
      axios
        .get(`${baseUrl}&pageSize=${pageSize}&startIndex=${startIndex}`)
        .then((response) => {
          const data = response.data.result;
          const totalRecords = response.data.totalRecords;
          const pageCount = this.handlePageCount(totalRecords, pageSize);
    ...
    //Paginacion
    //handlePageClick es una función que se ejecuta cuando se hace clic en un número de página en la paginación. 
